Shader "PlanetGeneration/PlanetVisuals"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Custom payload to return all PBR data at once
            struct BiomePayload
            {
                float3 color;
                float3 normalOffset;
            ...
            BiomePayload GetBiomeSurfaceData(SurfaceBiomeData biome, float3 positionOS, float3 positionWS, float3 normalWS, float steepness)
            {
                ...
                if (isCliff >= 1.0)
                {
                    ...
                    payload.smoothness = biome.smoothness * 0.5; // Cliffs are naturally rougher
                    payload.normalOffset = float3(0,0,0);
                    return payload;
                }

                // 2. 3D Vector Stretching
                ...
                // 4. Screen-Space Bump Mapping (Derivatives)
                ...
                // Construct surface gradient without extra noise samples
                float3 tangent = cross(dpdy, normalWS);
                float3 bitangent = cross(normalWS, dpdx);
                payload.normalOffset = (tangent * dhdx + bitangent * dhdy);
            ...
            Varyings vert(Attributes IN)
            {
                ...
                OUT.biomeIndices = uint3((uint)IN.blendData.x, (uint)IN.blendData.y, (uint)IN.blendData.z);
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // Apply the bump map modifier to the actual world normal
