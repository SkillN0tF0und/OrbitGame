...
namespace OrbitGame.OrbitMechanics
{
    ...
    public static class OrbitalMathUtility
    {
        ...
        public static OrbitalState CalculateOrbitalState(OrbitalElements elements, double time, double parentMass)
        {
            ...
            return CalculateOrbitalStateByAngle(elements, trueAnomaly);
        }

        // NEW: Generates a geometric point based purely on angle, bypassing time-distortion
        public static OrbitalState CalculateOrbitalStateByAngle(OrbitalElements elements, double trueAnomaly)
        {
            double a = elements.orbitDistance;
            double e = elements.orbitStretch;

            double radius = (a * (1.0d - e * e)) / (1.0d + e * Mathd.Cos(trueAnomaly));
